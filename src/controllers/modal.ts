...
function validate(validatableInput: Validatable): boolean {
  ...
  if (validatableInput.minLengthCharacters != null) {
    ...
    isValid = isValid
      && validatableInput.value.split(' ').filter(value => value !== '').every(value => value.length >= minLengthCharactersValue);
  }

  if (validatableInput.countWord != null) {
    isValid = isValid && validatableInput.value.split(' ').map(value => value.trim()).filter(value => value !== '').length >= validatableInput.countWord;
  }
  ...
  if (validatableInput.maxValueMonth != null) {
    if (validatableInput)
      isValid = isValid && Number(validatableInput.value.slice(0, 2)) <= validatableInput.maxValueMonth;
  }

  if (validatableInput.minValueYear != null) {
    if (validatableInput)
      isValid = isValid && Number(validatableInput.value.slice(3, 5)) >= validatableInput.minValueYear;
  }
...
export class ValidateProjectInput {
  ...
  constructor() {
    ...
    this.phoneNumberEl = <HTMLInputElement>document.getElementById('phoneNumber');
    ...
    this.cardNumberEl = <HTMLInputElement>document.getElementById('card-number');
    this.expirationDateEl = <HTMLInputElement>document.getElementById('expiration-date');
    this.cvvEl = <HTMLInputElement>document.getElementById('cvv');
    this.cardLogo = <HTMLImageElement>document.querySelector('.card__logo');
    this.errorMessage = <HTMLDivElement>document.querySelector('.error-message');
    this.formGroupClose = <HTMLDivElement>document.querySelector('.form-group__close');
  ...
  private validateCardNumberField(): void {
    this.cardNumberEl.addEventListener('input', (event: InputEventInit) => {
      if (!/[0-9]/.test(this.cardNumberEl.value) || this.cardNumberEl.value.length > 19) {
        this.cardNumberEl.value = this.cardNumberEl.value.slice(0, this.cardNumberEl.value.length - 1)
      }

      if (this.cardNumberEl.value.match(/[^\0-9\-]/)) {
        this.cardNumberEl.value = this.cardNumberEl.value.replace(/[^\0-9\-]/, '');
      };

      if (event.inputType === 'deleteContentBackward' && /[0-9]{4}/.test(this.cardNumberEl.value.slice(-4))) {
        this.cardNumberEl.value = this.cardNumberEl.value.slice(0, this.cardNumberEl.value.length - 1)
      }

      if (/[0-9]{4}/.test(this.cardNumberEl.value.slice(-4)) && this.cardNumberEl.value.length < 19) {
  ...
  private validateExpirationDateField(): void {
    this.expirationDateEl.addEventListener('input', (event: InputEventInit) => {
      if (!/[0-9]/.test(this.expirationDateEl.value) || this.expirationDateEl.value.length > 5) {
        this.expirationDateEl.value = this.expirationDateEl.value.slice(0, this.expirationDateEl.value.length - 1)
      }

      if (this.expirationDateEl.value.match(/[^0-9\/]/g)) {
        this.expirationDateEl.value = this.expirationDateEl.value.replace(/[^0-9\/]/g, '');
      };

      if (event.inputType === 'deleteContentBackward' && /[0-9]{2}/.test(this.expirationDateEl.value.slice(-2))) {
        this.expirationDateEl.value = this.expirationDateEl.value.slice(0, this.expirationDateEl.value.length - 1)
      }

      if (/[0-9]{2}/.test(this.expirationDateEl.value.slice(-2)) && this.expirationDateEl.value.length < 4) {
  ...
  private gatherUserInput(): string[] | void {
    ...
    if (!validate(nameValidatable) || !validate(phoneNumberValidatable) || !validate(emailValidatable)
      || !validate(addressValidatable) || !validate(cardNumberValidatable) || !validate(expirationDateValidatable)
